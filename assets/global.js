  ...
  summary.addEventListener('click', (event) => {
    ...
    if (summary.closest('details').querySelector('.details_smooth') && window.matchMedia('(max-width: 990px)')) {
      ...
      if (event.currentTarget.closest('details').hasAttribute('open')) {
        ...
        summary.closest('details').querySelector('.details_smooth').style['max-height'] = '55rem';
...
class ModalOpener extends HTMLElement {
  constructor() {
    ...
    const media_zoom = this.querySelector('.product__media');
    media_zoom?.addEventListener('click', () => {
      const modal = document.querySelector(this.getAttribute('data-modal'));
      if (modal) modal.show(media_zoom);
    });
  }
}
...
class SliderComponent extends HTMLElement {
  ...
  initCarousel(carousel) {
    var setClickable = this.setClickable(),
        ...
        setspaceBetweenLarge = this.dataset.spaceBetweenLarge ? this.dataset.spaceBetweenLarge : setspaceBetween;
    ...
    if (this.classList.contains('vertical')){
      ...
      var swiperOptions = {
        ...
        coverflowEffect: {
          rotate: 0,
          slideShadows: false,
          depth: this.dataset.cusScale ? 220 : 0,
          scale: this.dataset.cusScale ? this.dataset.cusScale : 0.8,
          stretch: 0,
        },
        pagination: {
      ...
      if (!this.classList.contains('swiper-more-item')) {
        ...
        var swiperOptions = {
          ...
          breakpoints: {
            ...
            2000: {
              slidesPerView: this.dataset.itemXxl,
              spaceBetween: setspaceBetweenLarge
            }
          },
          on: {
            init: function () {
...
class SlideshowComponent extends HTMLElement {
  constructor() {
    ...
    this.type = this.dataset.typePagination;
    this.value;
    this.carousel = this.querySelector('.swiper:not(.multiple-swipers)');
    this.mulCarousel = this.querySelectorAll('.swiper.multiple-swipers');

    this.mulCarousel.length > 0 ? this.initCarousel(this.carousel, this.mulCarousel) : this.initCarousel(this.carousel, false);
  ...
  setAutoPlay(swiper) {
    ...
    this.querySelectorAll('.swiper').forEach((carousel) => {
      if(this.sliderAutoplayButton) {
  ...
  initCarousel(carousel, multipleCarousel) {
    var setClickable = this.setClickable(),
        setTypePanigation = this.setTypePanigation(),
        setInfiniteScroll = this.classList.contains('infinite-scroll'),
        setAutoplaySpeed = this.dataset.speed * 1000;
    ...
    if (this.type == 'number') {
      ...
      if (multipleCarousel) {
        ...
        const thumbnails = this.querySelectorAll('[class*="multiple-swipers-"]');
        let list = [];
        thumbnails.forEach(element => {
          var multipleSwiper = new Swiper(element, multipleOptions);
          list.push(multipleSwiper);
          swiper.controller.control = list;
        })
      }
    }
  }
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${requestedVariantId}&section_id=${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        //Update product badge in product gallery
        var product_info = document.querySelectorAll('.product-section');
        if (product_info) {
          product_info.forEach((element) => {
            var priceMain = element.querySelector('.product__price');
            var priceMainChild = priceMain.querySelector('.price').className;
            const priceGallery = element.querySelector('[id^="PriceProductGallery-"]');
            priceGallery.className = priceMainChild;

            const saleDiscount = element.querySelector('.product__price .price-discount')?.dataset.discountPrice;
            const saleBadge = element.querySelector('.price__badge-sale');
            if (saleDiscount && saleBadge) saleBadge.textContent = saleDiscount;
          });
        }
      });
  }
